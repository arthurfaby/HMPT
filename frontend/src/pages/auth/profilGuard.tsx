...
function isCompleted(user: User): boolean {
  const profil = [
    ...
    { key: "age", keyFrench: "votre age" },
  ];

  const profilCompleted = profil.filter(
    (profil) =>
      !user ||
      !user[profil.key as keyof User] ||
      (profil.key === "interests" &&
        user[profil.key as keyof User] &&
        (user[profil.key as keyof User] as string[]).length === 0) ||
      (profil.key === "age" && user[profil.key as keyof User] === 0) ||
      (profil.key === "pictures" &&
        user[profil.key as keyof User] &&
        !verifyArrayPictures(user[profil.key as keyof User] as string[])),
  );
  if (profilCompleted.length === 0) return true;
...
export default function ProfilGuard(props: PropsWithChildren) {
  ...
  if (account && isCompleted(account)) return <Outlet />;
  else {
